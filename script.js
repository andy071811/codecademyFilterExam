...
const filterBooks = (searchInput, listOfBooks) => {
  let filteredBooks = [];
    let flatArr = flattenObjectValuesIntoArray(listOfBooks);
    for (let i = 0; i < listOfBooks.length; i++) {
        if (flatArr[i].includes(searchInput)) {
            filteredBooks.push(listOfBooks[i])
        }
    }
...
const structureBooksAsHtml = (bookList) => {
  ...
  for (let i = 0; i < bookList.length; i++) {
    newList = bookList.map(book => structureBookAsHtml(book))
  };
  return newList;
};

// Handler triggered when a user clickers the "Search" button. Chains previously defined functions together to filter books based on the search value, formats the books as HTML and renders them to the DOM
const searchBtnClickHandler = (listOfBooks) => {
  let searchValue = captureSearchValue();
    let filteredBooks = filterBooks(searchValue, listOfBooks);
